...
function openAdoptLorebookModal(ctx, chatId, books, send) {
...
function renderChapterPicker(host, c, ctx, send) {
  ...
  const help = document.createElement("div");
  help.className = "lmb-help";
  help.textContent = "Covered messages are already filed and are greyed. Click one endpoint, then click another to add the range between them. Use Exclude to pin a message so it's never hidden, replaced, or summarized - it splits compression around it.";
...
function buildMessageRow(m, c, onToggle) {
  ...
  cb.addEventListener("change", () => {
    if (cb.checked) {
      const anchorId = localState.anchorMessageId;
      const appliedRange = anchorId && anchorId !== m.id ? applyRangeSelection(c, anchorId, m.id) : false;
      if (!appliedRange)
    ...
    if (localState.anchorMessageId === m.id)
      localState.anchorMessageId = null;
...
function renderNameMacros(host) {
  ...
  const macros = [
    ...
    ["{{order}}", "Chronological lorebook order: 1, 2, 3..."],
...
function renderNaming(host, state, send) {
  ...
  addTemplate("Chapter entry", "chapterNameTemplate", "#{{order}} - {{title}} (msgs {{scene}})");
  addTemplate("Arc entry", "arcNameTemplate", "Arc {{padded}} - {{title}}");
  addTemplate("Volume entry", "volumeNameTemplate", "Volume {{padded}} - {{title}}");
  host.appendChild(sec.wrap);
}
function renderExtras(host, state, send) {
  ...
  const row = document.createElement("div");
  row.className = "lmb-actions";
  row.append(makeButton("Adopt existing lorebook", () => send({ type: "prepare_adopt_lorebook", chatId }), {
    disabled,
